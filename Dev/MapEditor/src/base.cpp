...
#define init_pointer(first,last,type) first = new type;last = new type; first->next = last;last->next = nullptr;
...
void CLS_WINDOWING::setWindowFlags(uint32_t x)
{
	window_flags = x;
}


void CLS_WINDOWING::setRenderFlags(uint32_t x)
{
	render_flags = x;
}
...
void applySurf(int x, int y, SDL_Surface *tex, SDL_Rect *clip, int w, int h)
{
	...
	temp = SDL_CreateTextureFromSurface(RENDER_MAIN, tex);
